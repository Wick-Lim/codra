...
import { installSessionAutoApprove } from "./session-auto-approve-production";
...
function pendingSession(overrides: Partial<RemoteSession> = {}): RemoteSession {
  return {
    ...
    protocolVersion: 1,
    requestedScopes: ["workspace.read", "agent.launch", "terminal.write"],
...
describe("installSessionAutoApprove (production)", () => {
  ...
  it('never approves a pending session, even when the test-only flag is "1"', async () => {
    process.env.CODRA_REMOTE_TEST_AUTO_APPROVE = "1";
    const { approve, registry } = harness();
    const reportError = vi.fn();

    const dispose = installSessionAutoApprove(registry, reportError);
  ...
  it("never approves a pending session when the flag is unset", async () => {
    delete process.env.CODRA_REMOTE_TEST_AUTO_APPROVE;
    const { approve, registry } = harness();
